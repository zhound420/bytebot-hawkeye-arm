...
# Normalize architecture (trim whitespace)
ARCH=$(echo "$ARCH" | tr -d '[:space:]')

# Debug output (helpful for troubleshooting)
echo -e "${BLUE}DEBUG:${NC} ARCH='$ARCH' OS='$OS'"
echo ""
...
if [[ ("$ARCH" == "arm64" || "$ARCH" == "aarch64") && "$OS" == "Darwin" ]]; then
    ...
    # Handle Linux ARM64 (DGX Spark or generic ARM64)
    echo -e "${GREEN}✓ Linux ARM64 ($ARCH) detected${NC}"
    ...
    # Handle x86_64/amd64 architectures
